-- ADITYA SUNIL KATHE T4 (22510061)
...
(1, 'Aditya Kathe', 101, '2023-01-15'),
(2, 'Abhay Jadhav', 102, '2023-02-20'),
(3, 'Sarthak Mahadik', 103, '2023-03-10');
...
INSERT INTO Customer (Customer_id, Customer_name, City_id, First_order_date) VALUES
(4, 'Rohit Patil', 104, '2023-04-05'),
(5, 'Arvind sharma', 105, '2023-05-12'),
(6, 'Aryan Shinde', 106, '2023-06-18');

-- INSERT INTO Customer (Customer_id, Customer_name, City_id, First_order_date) VALUES
-- (7, 'Amit Kulkarni', 107, '2023-07-22'),
-- (8, 'lalit Joshi', 108, '2023-08-30'),
-- (9, 'Vikas Pawar', 109, '2023-09-14');
...
(103, 'Kolhapur', 'HQ CH Address', 'IL', NOW());
...
-- 1. Find stores with a specific item (item_id = 102)
...
JOIN SalesDB.Stored_items si ON s.Store_id = si.Store_id
JOIN SalesDB.Items i ON si.Item_id = i.Item_id
WHERE i.Item_id = 102;

-- 2. Find orders from a specific store (store_id = 2)
...
-- 3. Find stores for a specific customer (customer_id = 1)
...
-- 4. Find headquarters with specific item inventory (item_id = 102, quantity > 2)
SELECT h.Headquarter_addr, h.City_name, h.State
FROM SalesDB.Headquarters h
JOIN SalesDB.Stores s ON h.City_id = s.City_id
JOIN SalesDB.Stored_items si ON s.Store_id = si.Store_id
WHERE si.Item_id = 102 AND si.Quantity_held > 2;

-- 5. List all orders with store and item details
...
-- 6. Find customer location (customer_id = 2)
...
-- 7. Get inventory for a specific item and city (city_id = 103, item_id = 104)
SELECT s.Store_id, h.City_name, i.Description, SUM(si.Quantity_held) AS Total_Stock
FROM SalesDB.Stores s
JOIN SalesDB.Headquarters h ON s.City_id = h.City_id
JOIN SalesDB.Stored_items si ON s.Store_id = si.Store_id
JOIN SalesDB.Items i ON si.Item_id = i.Item_id
WHERE h.City_id = 103 AND i.Item_id = 104
GROUP BY s.Store_id, h.City_name, i.Description;

-- 8. Get detailed order information
...
-- 9. Customer Types Queries

-- Walk-in Customers
SELECT c.Customer_id, c.Customer_name, w.tourism_guide
FROM HeadquarterDB.Customer c
JOIN HeadquarterDB.Walk_in_customers w ON c.Customer_id = w.Customer_id;

-- Mail-order Customers
SELECT c.Customer_id, c.Customer_name, m.post_address
FROM HeadquarterDB.Customer c
JOIN HeadquarterDB.Mail_order_customers m ON c.Customer_id = m.Customer_id;

-- Dual Customers (Both Walk-in and Mail-order)
